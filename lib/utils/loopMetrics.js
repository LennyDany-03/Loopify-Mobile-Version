...
export function buildWeeklyBars(trendData = [], count = 7) {
  const recentWeeks = trendData.slice(-count);
  const maxCount = Math.max(...recentWeeks.map((item) => item.count || 0), 0);
  ...
  return recentWeeks.map((item) => ({
    ...
    height: maxCount > 0 ? Math.max(Math.round(((item.count || 0) / maxCount) * 100), 12) : 12,
  }));
}
