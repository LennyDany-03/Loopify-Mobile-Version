...
export function getLoopProgress(loop, todayCheckins = {}) {
  if (!loop) {
    return 0;
  }
...
export function normalizeLoop(loop, options = {}) {
  ...
  return {
    ...
    completion_rate: getLoopProgress(loop, todayCheckins),
  };
}
...
export function getRecentWeekKeys(count = DEFAULT_WEEK_COUNT, referenceDate = new Date()) {
  const seen = new Set();
  const keys = [];
...
function getOrderedWeekKeys(loopWeeklyData = [], count = DEFAULT_WEEK_COUNT, referenceDate = new Date()) {
  ...
  loopWeeklyData.forEach(({ weeks = [] }) => {
    weeks.forEach((entry, index) => {
      ...
      const existing = weekMeta.get(entry.week);
      const parsedStartDate = entry.start_date ? Date.parse(entry.start_date) : Number.NaN;

      weekMeta.set(entry.week, {
        week: entry.week,
        order: Number.isFinite(parsedStartDate)
          ? parsedStartDate
  ...
  if (!weekMeta.size) {
    return getRecentWeekKeys(count, referenceDate);
  }

  return Array.from(weekMeta.values())
    .sort((left, right) => {
...
export function aggregateWeeklyCounts(
  loopWeeklyData = [],
  count = DEFAULT_WEEK_COUNT,
  referenceDate = new Date()
) {
  const weekKeys = getOrderedWeekKeys(loopWeeklyData, count, referenceDate);
...
export function buildRecentLoopInsights(
  loopWeeklyData = [],
  count = 3,
  referenceDate = new Date()
) {
  const weekKeys = getOrderedWeekKeys(loopWeeklyData, 2, referenceDate);
  const previousWeekKey = weekKeys[weekKeys.length - 2];
